# AGENTS.md

This is the `@aivin-labs/sdk` package itself (the runtime client library) - not a plugin project. If
you're instead working *inside* a project scaffolded by `aivin create`, that project has its own
`AGENTS.md` already; this one is for people/agents working on the SDK's own source.

The interactive `aivin` CLI (`create`/`init`/`deploy`/`login`/...) lives in a separate sibling
repo/package, `@aivin-labs/cli` - not here. This package ships only `bin/server.mjs` (exposed as
the `aivin-server` bin entry), the process a deployed plugin container actually runs via
`npm start` - see `docs/CHANGELOG.md`'s "the aivin CLI moved to @aivin-labs/cli" entry for why.

## Layout

- `src/` - the published library (compiled to `dist/`). `src/sdk/SDKClient.ts` is the whole
  `import { ai } from '@aivin-labs/sdk'` surface (also reachable via the legacy `ctx.sdk`); `src/grpc/GrpcInvoker.ts` is the
  transport (retry/backoff, streaming, tracing hooks); `src/PluginServer.ts` is the gRPC server a
  deployed plugin runs - it now hands the actual `main()` invocation to `src/worker/PluginWorkerHost.ts`,
  which runs the plugin's code in a sandboxed worker thread (`src/worker/PluginWorkerRuntime.ts`,
  protocol in `src/worker/protocol.ts`) that never sees the container's real secret, relaying every
  SDK call back over `postMessage`; `src/pluginLoader.ts` resolves/loads the plugin module for that
  worker; `src/sdk/ContextBuilder.ts` builds an explicit flow-execution context (agent/workspace/
  session) for `agent.runFlow()`; `src/sdk/trace.ts` is the per-invocation execution trace.
- `bin/server.mjs` - real ESM, not part of the compiled `dist/`, runs directly. This is what
  `aivin-server` (this package's own bin entry) and `aivin start` (from the separate
  `@aivin-labs/cli` package) both ultimately spawn.
- `docs/` - the real reference. `docs/SDK.md` + `docs/sdk/*.md` (one file per namespace) is the
  full SDK surface; `docs/MANIFEST.md` is `manifest.json`'s field reference (CLI command reference,
  `docs/CLI.md`, now lives in the `@aivin-labs/cli` repo); `docs/AI-Plugin-Guide.md` is the
  condensed cheat sheet for an AI generating a plugin quickly; `docs/CONTEXT.md` documents the `ctx`
  handler argument; `docs/DATA_STRUCTURES.md` covers the exported types mirroring the backend's real
  data models; `docs/EXAMPLES.md` is real plugins across common use cases;
  `docs/PLUGIN_DEVELOPMENT_GUIDE.md` is the end-to-end empty-folder-to-deployed walkthrough;
  `docs/SINGLE_METHOD_PLUGINS.md` covers the handler pattern; `docs/ARCHITECTURE.md` is internals
  documentation for people working on the SDK itself - not needed to just write a plugin
  (auth/infra internals live outside this public repo); `docs/draft/` holds in-progress design
  docs not yet finalized (e.g. the worker-sandbox design); `docs/CONTRIBUTING.md`,
  `docs/CHANGELOG.md`, and `docs/COMPETITIVE_LANDSCAPE.md` are project-facing, not SDK-usage reference.
- `test/` - `node:test`, run via `npm test`. No mocking framework - fakes are passed in directly
  (see `SDKClientOptions.invoke`/`invokeStream` and `GrpcInvoker.emitTraceForTest`).

## Conventions specific to this repo

- Param shapes in `SDKClient.ts` are verified against the backend's real implementation in the
  separate backend repo, **not** the backend's own declared type contract - that declared type
  file has drifted from the real backend in several places. Doc comments on individual methods flag
  where a shape differs from what that declared contract would suggest.
- The `@aivin-labs/cli` repo's scaffold generator (`createPackageJson`) pins a fresh plugin's
  `@aivin-labs/sdk` dependency to an **exact version** (not `latest`/a range - the platform's own
  deploy-time security scan flags non-exact pins as a supply-chain risk), resolved automatically
  from whatever `@aivin-labs/sdk` version the CLI itself depends on via this package's
  `"./package.json"` export - nothing to bump here manually.
- Use `import { ai, ... } from '@aivin-labs/sdk'` in every example, generated scaffold, and doc
  snippet - never `ctx.sdk.*` or a default `import SDK from '@aivin-labs/sdk'`. `ctx.sdk` is the
  legacy mechanism: same client, still supported, documented only as legacy (its one niche is code
  that isn't guaranteed to run inside `main()`).
- The default `manifest.json` shape is `{ ...sharedFields, plugins: [...] }` - one entry per
  function, each with a required `func` naming its export (`"main"` in the scaffold). The flat
  single-object manifest is legacy (still accepted; proxy manifests keep it).

## Before publishing

`npm run build && npm run lint && npm test` must all pass. `prepublishOnly` already runs
clean+build, but lint/test are not wired into it - run them yourself first.
